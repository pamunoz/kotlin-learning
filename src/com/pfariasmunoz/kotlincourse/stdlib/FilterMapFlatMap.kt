...
val albums = listOf(
        ...
        Album("A Souceful of Secrets", 1968, 9, 0),
        Album("More", 1969, 9, 153))

fun albumAndTrackLowerThanXSeconds(durationInSeconds: Int, albums: List<Album>): List<Pair<String, String>> {
    // the flatMap apply to a collection of collection functions an flaten them creating a single collection
...
fun main(args: Array<String>) {
    ...
    // Inproving the function above, adding filter an chaining it
    albums.filter { it.chartUK == 1}
            .forEach { println("Now Found it: ${it.title}") }

    // And another better way using map
